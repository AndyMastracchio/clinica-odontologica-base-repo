...
public class IntegracionTurnosTest {
    ...
    public void cargarDataSet() throws ResourceNotFoundException, BadRequestException {
        ...
        pacienteService.guardar(new Paciente("Santiago", "Paz", "88888888", new Date(), domicilio));
        odontologoService.registrarOdontologo(new Odontologo("Santiago", "Paz", 3455647));
        Paciente paciente = pacienteService.buscar(1L);
        Odontologo odontologo = odontologoService.buscar(3L);
                turnoService.registrarTurno(
                new Turno(paciente, odontologo, new Date()));
    }
